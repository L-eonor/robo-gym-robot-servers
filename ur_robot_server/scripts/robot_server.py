...
from ros_bridge import UrRosBridge, UrReachRosBridge
...
class RobotServerServicer(robot_server_pb2_grpc.RobotServerServicer):
    def __init__(self, real_robot, ur_model, has_gripper, number_of_joint_positions=7, number_of_joint_velocities=7):
        if( has_gripper):
            self.rosbridge = UrReachRosBridge(real_robot= real_robot, ur_model= ur_model, number_of_joint_positions=number_of_joint_positions, number_of_joint_velocities=number_of_joint_velocities)
        else:
